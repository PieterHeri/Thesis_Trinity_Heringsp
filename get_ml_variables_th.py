...
Determining average swing, stance, stride time and Coefficient of variability  from raw Shimmer 3 IMU data 

Pieter Herings
Trinity College Dublin, University of Dublin

read README (word file) for a walkthrough of the code and common errors
"""
...
def get_data_patient(mat_pt, shimmer_column):
    ...
    timestamps = (x - x[0])*1000                                               #adjust timestamps so it starts at 0 and is in milliseconds
                                       #sample rate
    #resample sig
    walktime = timestamps[-1] /1000
    dpoints = int(sethz*walktime)
    resampsig = sc.signal.resample(sp_shimmer_output, dpoints)  
    tresamp = sc.signal.resample(timestamps, dpoints)

    hertz = len(resampsig) / (tresamp[-1]/1000)
    return tresamp, resampsig, hertz, walktime
...
def filt_corr_data(patientfile, ctbu):
    ...
    time, xgyro, hz, walkt = get_data_patient(patientfile, datacolumn)                #take specific shimmer data from patientfile
...
def average_sss(sw, sta, st):
    ...
    for i in range(len(bb)):                                                   #for each of the lists (swing times etc.)
        ...
        c2 = c1[c1<highl] 
        std_box.append(stdev(c2)/1000)                                                     #take only values lower than higher limit in c1 and name c2
        cor_avr = np.average(c2)/1000                                          #take average (corrected now) in seconds
        cor_av_list.append(cor_avr)                                            #append average to list 
    
    return cor_av_list, std_box
...
def get_sss(ptfile_mat, cutff, multf_msv, multf_tohs):
    ...
    """
    t, sg, h, wt = filt_corr_data(ptfile_mat, cutff)                           #take filtered and corrected (mounterror) timearray, signalarray and hertz from matlabfile with cutoff value butterworth filter as input
    ...
    dbox_av_sss, variabilities = average_sss(swing_times, stance_times, stride_times)         #filter out values > 2*std and take average (thus 3 in total)
    
    stride_time_variability = stdev(stride_times) / np.mean(stride_times) * 100
    ...
    swi, stan, stri = dbox_av_sss   
                                         #take averages out of dbox
    vswi, vstan, vstri = variabilities
    with open('Dual_left_ml_variables.csv', 'a+') as file:   	                           #open a vsc file with particular name (variable 1 after open), 'a+' determines that if this file is non-existent, create one

        #file.write(str("'ptno', 'cutoff', 'hz', 'av_swi', 'av_sta', 'av_str', 'var_swi', 'var_sta', 'var_str', 'walktime', 'speed'"))           #ONLY FIRST PATIENT YOU TEST, first row of csv file will name your variables#enter
        file.write(str('\n'))
        file.write(str(patientno))                        #write down variables in csv file on new line
        file.write(',')
        file.write(str(cutff))
        file.write(',')
        file.write(str(int(h)))
        file.write(',')
        file.write(str(swi))
        file.write(',')
        file.write(str(stan))
        ...
        file.write(str(vswi))
        file.write(',')
        file.write(str(vstan))
        file.write(',')
        file.write(str(vstri))
        file.write(',')
        file.write(str(wt))
        file.write(',')
        file.write(str(speed))
        file.write(',')
        file.write(str(stride_time_variability))

    return dbox_av_sss


#------------------------------------------------------------------------------
get_sss('E001LS.mat', 0.05, 1.5, 0.3)
