...
Determining average swing, stride and stance with 'get_sss' times from Shimmer 3 IMU data 
Secondly, determine complexity index with 'complexity_index' from raw Shimmer 3 data.
...
def get_sss(ptfile_mat, cutff, multf_msv, multf_tohs):
    ...
    #with open('Initial_left.csv', 'a+') as file:   	                       #open a vsc file with particular name (variable 1 after open), 'a+' determines that if this file is non-existent, create one

        #file.write(str(["Patientno","cutoff value", "multf_msv", 
                        #"multf_tohs", "Swing", "Stance", "Stride"]))           #ONLY FIRST PATIENT YOU TEST, first row of csv file will name your variables
        #file.write("\n")                                                       #enter
        #file.write(str([patientno, cutff, multf_msv, 
                        #multf_tohs, swi, stan, stri]))                         #write down variables in csv file on new line
...
def multiscale_entropy(time_series, sample_length, tolerance = None, maxscale = None):
    ...
    for i in range(maxscale):
        ...
        x_axis = np.array(range(0,6298))
        
        y_axis = sc.signal.resample(temp*-1, 6298)
        
        if i+1 == 1 or i+1 == maxscale:
            plt.plot(x_axis[4000:4200], y_axis[4000:4200], label='scale = {}'.format(i+1), color=[0,0.2,i*(1/maxscale)])
        else:
            plt.plot(x_axis[4000:4200], y_axis[4000:4200], color=[0,0.2,i*(1/maxscale)])



    plt.ylabel('Angular velocity (m/s)', size=15)
    plt.xlabel('Time(s)', size=15)
    plt.legend(fontsize=15)
    

    plt.savefig('granulating_samples_patient128.png')
    #plt.figure(figsize=(10, 5))
    #plt.plot(range(maxscale), mse, label= 'Sample entropy', color= [0.4, 0.3, 0.9])
    #plt.xlim([0, 40])
    #plt.ylim([3,6])
    #plt.xlabel('Scale factor', size=15)
    #plt.ylabel('Sample entropy', size=15)
    #plt.savefig('entropy_figure_patient128.png')
    return mse
